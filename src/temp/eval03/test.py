...
def get_func_name(file_name):
  ...
  while True:
    line = f.readline()
    if not line or "Dwarf_parsing_error" in line : break
    name = line.split(" ")[0]
    func_list.append(name)
  return func_list


def parse_count(file_name, func_name):
  func_count =0
  total_bbl = 0
  pass_bbl = 0
  f = open(file_name,"r")
  ...
  while True:
    ...
    data = line.split(" ")
    func = data[0]
    if func[1:-1] not in func_name: continue
    func_count += 1
    total_bbl = total_bbl + int(data[1])
    pass_bbl = pass_bbl + int(data[2])
  f.close()
  return total_bbl,pass_bbl
...
if __name__ == '__main__':
  ...
  for i in files:
    if ".vsa" in i:
      ...
      total,pass_ = parse_count(i, func_name)
      if total == 0 and pass_ == 0: continue
      string = "%s,%s,%s,%s\n" % (i,pass_, total, (pass_*100/total))
      f.write(string)
      result.append([pass_, total, (pass_*100/total)])
    else: continue
  a = 0
  b = 0
  c = 0
  for i in result:
    ...
    b = b + i[1]
    c = c + i[2]
  string = "total,%s,%s,%s" % (a,b,(c/len(result)))
  f.write(string)
  f.close()
