...
if __name__ == '__main__':
  no_files = ['evaluate_vsa.py','ida_script.py','get_dwarf_info.py','test.fsproj','Program.fs', 'bin', 'obj','.ida','.vsa','.result','.dwarf','i64','test']
  # 1.save the result of get_dwarf
  # 2.save vsa
  # 3.make aloc of get_dwarf (offset-16)
  # 4.parse aloc of vsa
  ...
  for i in files:
    print(i)
    if i in no_files or "i64" in i or "ida" in i or "vsa" in i or "result" in i or "dwarf" in i or "test" in i:
      continue
  ...
  for i in result:
    ...
    f.write(string)
    if result[i][0] == 0 and result[i][2] ==0:
      continue
    count =  count + 1
    a = result[i][0] + a
    b = result[i][1] + b
    c = result[i][2] + c
    d = result[i][3] + d
  string = "%s %s %s %s" % (a/count, b/count, c/count, d/count)
  f.write(string)
  f.close()
