...
def parse_count(file_name,w,ww):
  best1 = 0
  best2 = 0
  total = 0
  f = open(file_name,"r")
  ...
  while True:
    line = f.readline()
    if not line: break
    elif "Dwarf_parsing" in line: break
    func = line.split("/")[0].split(" ")[0]
    a = line.split("/")[0].split(" ")[-1]
    b = line.split("/")[1].split(" ")[0]
    c = line.split("/")[1].split(" ")[-1]
    string = "%s,%s,%s,%s,%s\n" % (file_name,func,a,c,b)
    w.write(string)
    best1 += int(a)
    total += int(b)
    best2 += int(c)
  f.close()
  string = "%s,%s,%s,%s\n" % (file_name,best1,best2,total)
  ww.write(string)
  return best1,best2,total
...
if __name__ == '__main__':
  ...
  data = []
  f = open("func","w")
  f.write("file,func,vsa,ida,total")
  ff = open("file","w")
  a = 0
  b = 0
  c = 0
  for i in files:
    if "elf.result" in i:
      vsa,ida, total = parse_count(i,f,ff)
      a += vsa
      b += ida
      c += total
    else: continue
  string = "total,%s,%s,%s" % (a,b,c)
  ff.write(string)
  f.close()
  ff.close()
